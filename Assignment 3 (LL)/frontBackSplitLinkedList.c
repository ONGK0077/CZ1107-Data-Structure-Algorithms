...
void frontBackSplitLinkedList(LinkedList *ll, LinkedList *resultFrontList, LinkedList *resultBackList)
{
	/* add your code here */
        ListNode *link_list;
        int link_size, frontSize, countFront, countBack;
        ...
        if (link_size < 2){
            if (link_size == 1){
                insertNode(resultFrontList,countFront,link_list->item);
            }
            return;
        }

        if(link_size%2 == 0){
            frontSize = link_size/2;
        }
        else{
            frontSize = (link_size/2)+1;
        }

        for (int i = 0; i < link_size; i++){
            if(frontSize>countFront){
                insertNode(resultFrontList,countFront,link_list->item);
                countFront++;
            }
            else{
                insertNode(resultBackList,countBack,link_list->item);
                countBack++;
            }
